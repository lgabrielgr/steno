...
@MainActor
@Test("two overlapping passes do not both apply: the second's result is superseded")
func aConcurrentPassDoesNotDoubleApply() async throws {
    let fixture = try RefreshFixture()
    let task = try fixture.task("ship payments")
    try fixture.ref("PAY-421", on: task)
    let taskID = task.id
    ...
    // is still in flight, so two services run over one context. Both snapshot the
    // ref with the same `since`; without the guard both apply, producing two
    // first-observation events for one ref (Copilot, PR #42).
    //
    // **The 80ms delay is margin, not the mechanism.** What orders this is that
    // both passes read their rows before either reaches its write phase: `async
    // let` starts both eagerly and each suspends at its task group, so the second
    // carries the pre-write `observedAt`. The delay widens that window so a loaded
    // CI runner cannot interleave a whole first pass in between.
    //
    // Removing the delay leaves this test passing today — checked by mutation — so
    // it is not load-bearing here, only defensive. Do not read it as the reason the
    // ordering holds.
    let connector = StubSourceConnector(
    ...
    async let launchPass = first.refresh(taskIDs: [taskID])
    async let preparePass = second.refresh(taskIDs: [taskID])
    let outcomes = await [launchPass, preparePass]

    // Exactly one event and one cache write, whichever pass won the apply.
    ...
    // Both fetched: the guard is in the write phase, not a reason to skip work.
