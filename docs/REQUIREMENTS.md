...
- **Cache:** persist `cachedSummary` as the durable last-known state of the resource. It has two jobs: it is the baseline a later fetch's changes are described against, and it is what the app shows when it tells the user their integration data is stale. **Last-known state reaches a *report* as `externalUpdate` events** (§3.3), which the report engine already collects for the window — not by injecting the cached summary into the report text or the AI prompt, which would couple the source layer to §7.3 and breach §13. A report therefore generates fully offline, and the staleness label is an app surface, not a line in the copied markdown.
...
**Mutable-field conflict rules.** These four groups of fields aren't append-only and need deterministic resolution (the two boolean flags above are a fifth case, resolved by D-098):
...
| `Project.lastStandupAt` | **Derive** from the project's reports: the latest of `windowEnd` for a live report and `windowStart` for an undone one | Reporting is a historical fact, so the clock must never move backwards on its own — but FR-4.1's undo moves it backwards *deliberately*, and stamps nothing a comparison could see. Comparing timestamps would therefore let any older export defeat an undo. Deriving reproduces exactly what `commit` and `undo` each write (D-099) |
...
{
  "schemaVersion": 1,
  "exportedAt": "2026-08-10T14:22:05.123Z",
...
- Timestamps carry milliseconds (`2026-08-10T14:22:05.123Z`). §10.1 resolves three of its four mutable-field merge rules by comparing timestamps, so whole-second precision would make two notes typed in the same second a tie no rule can break, and their order in the append-only log unrecoverable. Encoding **rounds to the nearest millisecond**, so a round-trip is accurate to within 0.5 ms in either direction and exact only when the fractional second is an eighth (`0`, `.125`, `.25`, `.375`, `.5`, `.625`, `.75`, `.875`) — the only values both exactly representable in binary and exactly expressible in three decimals. Rounding rather than truncating is what makes the format a **fixed point**: a value read out of a file and written back must produce the same digits, or a timestamp drifts a little every time it crosses a machine and §10.6's idempotency property is false. Truncation does not have that property — measured, 496 of 1000 millisecond values move (D-101).
- **Keys are emitted in alphabetical order; the example above groups them for readability, not in emitted order.** Sorting is what makes the byte output deterministic: without it the key order is the encoder's internal dictionary order, which differs between processes, so two exports of an unchanged store would be byte-different files — defeating the diffability this section asks for and reducing §10.5's auto-export history to noise. JSON objects are unordered by definition, so no reader is affected.
- `Event.payload` is carried as **base64**, not as nested JSON, and is therefore the one part of the file that is not greppable. This is not hypothetical: FR-4's Copy writes a payload on every `standupReported` event, so ordinary stores have many. Nesting the JSON would mean re-serializing it on import, which does not reproduce the original bytes — and every field surviving byte-exactly outranks readability when there is no second copy.
...
- Merge divergent stores in both orders → identical final state (commutativity). **Stated over the record set and every field except the two cached ones.** `SourceRef.lastFetchedAt` and `.cachedSummary` are excluded from the export by default (§10.2), so an ordinary export does not describe them at all — and §10.1's "`nil` loses to any value" then preserves whichever machine is the import *target*, which is not the same in both directions. That is not a defect in the merge rule: the file genuinely carries no information about those two fields, and §10.2 excludes them precisely because they are bulky and re-fetchable. Convergence over them is a property of exports taken with `includesCachedExternalData` set, and is tested that way (D-105).
