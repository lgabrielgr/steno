...
- [ ] **Step 7: Prove the tie-break is load-bearing**

Change the event comparator to `lhs.timestamp < rhs.timestamp`, dropping the id.
Run: `make test`
Expected: `two events sharing a timestamp order by id, whichever went in first` fails. Restore the comparator and re-run to green.
...
## Task 3: Completeness — two allowlists, and the values

The task file's first acceptance criterion is that every field of every model survives. Three tests, failing for three different reasons — and the second one exists because mutation testing proved the first misses the case §10.3 actually names.
...
The middle row is the finding this task exists to encode: a new optional field is `nil`, `encodeIfPresent` omits its key, and the emitted JSON is byte-for-byte unchanged. A field with no fixture value is never anything but `nil`, so the JSON assertion alone would never see it.
...
| D-095 | §10.3's assertion needs two tests, because the JSON key allowlist provably misses a new optional field — it is `nil`, `encodeIfPresent` omits the key, the bytes are unchanged. A `Mirror` assertion over declared properties is the one that catches it. Mutation-tested |
...
| D-097 | `Event.payload` exports as base64 for now. It is `nil` for every event the app creates; embedding it as nested JSON belongs to the task that first writes one |
