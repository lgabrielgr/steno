...
- Produces:
  - `ExportEncoder` — `@MainActor struct`, `init(context: ModelContext, includesCachedExternalData: Bool = false, now: @escaping () -> Date = Date.init, exportedBy: String = ExportDocument.userAgent())`, `func snapshot() throws -> ExportDocument`, `func encode() throws -> Data`, two `static func precedes(_:_:)` overloads (projects and source refs), and `static func sortedByWireInstant<Element>(_:instant:id:) -> [Element]` for the three date-ordered arrays.
...
Drop `.formatted(ExportDocument.fractionalSeconds)` from `sortedByWireInstant`, so the key is the in-memory `Date`.
Expected: `the export order survives a round trip, even below the wire precision` fails. This is the one that is invisible without the test — two events a fraction of a millisecond apart sort correctly *here* and identically on the wire, so a store built from the file ties on them and reverses the pair.

Drop the `tieBreak` from the same sort, leaving only the key.
Expected: `two events sharing a timestamp order by id, whichever went in first` fails.

Change the project comparator to `(lhs.sortOrder, lhs.id.uuidString)`, dropping `name`.
Expected: `projects sharing a sortOrder fall back to name, as the sidebar does` fails. `sortOrder` is not unique and `MainWindowModel.fetchProjects` breaks that tie on `name`, so this is the difference between exporting in the order the user sees and one nobody has ever seen.

Restore all three and re-run to green.
...
Expected: `timestamps carry milliseconds, not whole seconds`, `an eighth of a second survives exactly` and `§10.1: two events a millisecond apart stay distinct and ordered` all fail — the last one because both events collapse onto the same instant. Restore and re-run to green.
...
- Timestamps carry milliseconds (`2026-08-10T14:22:05.123Z`). §10.1 resolves three of its four
  ...
  round-trip is accurate to within 1 ms and exact only when the fractional second is an eighth
  (`0`, `.125`, `.25`, `.375`, `.5`, `.625`, `.75`, `.875`) — the only values both exactly
  representable in binary and exactly expressible in three decimals.
```
...
| D-091 | Timestamps carry milliseconds and encoding **truncates**, so a round-trip is within 1 ms and never later, and exact only when the fractional second is an eighth — not every dyadic value, since `.0625` truncates to `.062`. Points at REQUIREMENTS v1.16 |
...
- [ ] **Step 5: Check that no document cites a test that does not exist**

Renaming a test silently invalidates every prose reference to it, and the refresh that rewrites the plan's code blocks does not touch prose. This catches that class:

```bash
python3 - <<'EOF'
import pathlib, re
names = set()
for f in pathlib.Path("StenoTests/Portability").glob("*.swift"):
    names |= set(re.findall(r'@Test\(\s*\n?\s*"([^"]+)"', f.read_text()))
def words(s): return set(re.findall(r"[a-zA-Z]+", s.lower()))
for label, path in {"plan": "docs/superpowers/plans/2026-09-11-m2.5-01-export-encoder.md",
                    "spec": "docs/superpowers/specs/2026-09-11-m2.5-01-export-encoder-design.md",
                    "decisions": "docs/DECISIONS.md"}.items():
    prose = re.sub(r"```.*?```", "", pathlib.Path(path).read_text(), flags=re.S)
    for c in set(re.findall(r"`([^`\n]{12,90})`", prose)) - names:
        cw = words(c)
        if len(cw) < 3: continue
        for n in names:
            if len(cw & words(n)) / max(len(cw | words(n)), 1) >= 0.5:
                print(f"STALE {label}: {c!r} ~ {n!r}")
EOF
```

Expected: no output. A hit means a document names a test that was renamed — it flags citations that are *near* a real name rather than absent from it, because a rename is the failure mode and an outright invented name is not.
