...
enum StoreMerge {
    /// Internal, not public: `ImportService` is the surface M2.5-03 and M2.5-04
    /// consume, and the merge takes values that only the service knows how to
    /// obtain at the right precision.
    ///
    /// Throws **every** value-level validation, not only §7's closure and
    /// consistency checks: duplicate record ids, an invalid cache pair, and an
    /// inverted report window all reject here too. That matters to a caller
    /// bypassing `ImportReader`, which is the only reason these live in two
    /// places.
...
| 6 | The save | `.saveFailed(detail:)` |
| — | Reading the existing store, at any point above | `.storeUnreadable(detail:)` |
...
New `StenoTests/Portability/` files, as they ended up: `MergeFixture.swift`,
`StoreMergePropertyTests.swift`, `StoreMergeRuleTests.swift`, `ImportRoundTripTests.swift`,
`ImportRejectionTests.swift`, `ImportApplyScopeTests.swift`, `ImportFieldCoverageTests.swift`,
`LastStandupClockTests.swift`, `WireInstantTests.swift`.

> The planned `ImportFixture`, `ImportReaderTests` and `ImportApplyTests` are not among them.
> Reader failures are asserted through the service in `ImportRejectionTests`, because the message
> a user sees is the thing worth pinning; `ImportApplyScopeTests` covers what `apply` is allowed
> to touch, which is a question the original plan did not know to ask.
