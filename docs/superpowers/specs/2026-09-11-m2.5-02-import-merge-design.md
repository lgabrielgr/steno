...
**`ImportPlan` carries a write set and a fingerprint.** §8 as written has neither. The counts and
...
The fingerprint makes `apply` refuse a plan whose store moved underneath it, through a new
`ImportError.storeChanged`. §3's note calling that gap theoretical was wrong: M2.5-03 puts a
preview on screen for as long as the user leaves it open.
...
apply and was discarding without a word. And both checks inside `StoreMerge` as well as in
`ImportReader`, because the merge takes values and does not know whether they came through the
reader.
